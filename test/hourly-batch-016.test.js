...
const expectedIds = [
  'assemblyloadcontext-collectible', 'ratelimiter-token-bucket', 'meter-counter',
  'process-argumentlist', 'appcontext-basedirectory', 'ipaddress-tryparse',
  'runtimefeature-dynamic-code', 'nativelibrary-load', 'x509certificate2-createfrompem',
  'dns-gethostaddressesasync'
];
const readJson = async path => JSON.parse(await fs.readFile(new URL(path, import.meta.url), 'utf8'));

test('hourly batch 016 contains ten diverse localized articles', async () => {
  const base = await readJson('../public/content/articles/hourly-batch-016.json');
  const ja = await readJson('../public/content/locales/ja/hourly-batch-016.json');
  const en = await readJson('../public/content/locales/en/hourly-batch-016.json');
...
test('batch 016 is loaded while the learning map remains curated', async () => {
  const loader = await fs.readFile(new URL('../src/content-loader.js', import.meta.url), 'utf8');
  assert.match(loader, /hourly-batch-016\.json/);
